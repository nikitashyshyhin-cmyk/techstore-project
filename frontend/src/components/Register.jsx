...
const Register = () => {
  ...
  const [isLoading, setIsLoading] = useState(false); // Добавили стейт загрузки
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Если всё ок — на страницу логина
        navigate('/login');
      }
    } catch (err) {
      // Пока просто логируем ошибку в консоль
      console.error("Ошибка при подключении к API", err);
    } finally {
