      ...
      <div className="bg-white w-full max-w-[950px] min-h-[600px] rounded-[2.5rem] shadow-2xl flex overflow-hidden relative">
        ...
        <div className="flex-1 p-10 md:p-14 flex flex-col items-center justify-center">
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-[320px] space-y-4">
            <input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl outline-none focus:border-[#4B32B1] transition-all placeholder:text-gray-300" />
            <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl outline-none focus:border-[#4B32B1] transition-all placeholder:text-gray-300" />
            <div className="flex items-center justify-between text-[10px] sm:text-xs py-1">
              <label className="flex items-center gap-2 text-gray-500 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-gray-300 accent-[#4B32B1]" /> Запам'ятати мене
              </label>
              <a href="#" className="text-gray-400 hover:text-[#4B32B1] underline">Забули пароль?</a>
            </div>
            <button type="submit" className="w-full bg-[#4B32B1] text-white py-3 rounded-xl font-medium shadow-md transition-all active:scale-[0.98] hover:bg-[#3b2888]">
              Увійти
            </button>
          </form>
