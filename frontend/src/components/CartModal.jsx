...
const CartModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm p-4 transition-opacity"
      onClick={onClose} // Закриття по кліку на темний фон
    >
      <div 
        className="bg-gray-50 w-full max-w-[900px] max-h-[90vh] rounded-md shadow-2xl flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()} // Запобігаємо закриттю при кліку на саме вікно
      >
        ...
        <div className="p-6 overflow-y-auto flex-1 hide-scrollbar">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-10 bg-white rounded-md border border-gray-100 shadow-sm">
              ...
              <button onClick={fetchCart} className="text-[#4B32B1] font-semibold hover:underline">
                Спробувати знову
              </button>
            </div>
          ) : cartItems.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-md border border-gray-100 shadow-sm">
              ...
              <button onClick={onClose} className="bg-[#4B32B1] text-white px-6 py-2.5 rounded-md font-semibold hover:bg-[#3b2888] transition-colors">
            ...
            <div className="space-y-3">
              {cartItems.map((item) => {
                ...
                return (
                  <div 
                    key={item.id} 
                    className={`bg-white p-4 rounded-md shadow-sm border border-gray-100 flex flex-col sm:flex-row items-center gap-4 transition-all ${isDeleting ? 'opacity-50 pointer-events-none' : 'hover:shadow-md'}`}
                  >
                    ...
                    <div className="flex-1 w-full text-center sm:text-left">
                      <Link
                        ...
                        onClick={onClose}
                        className="font-semibold text-gray-700 text-sm hover:text-[#4B32B1] transition-colors line-clamp-2"
                      >
                    ...
                    {/* СУМА */}
                    <div className="font-bold text-[#4B32B1] whitespace-nowrap sm:w-28 text-right">
                    ...
                    <button
                      ...
                      className="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors disabled:opacity-50 flex-shrink-0"
                    >
                      {isDeleting ? (
          ...
          <div className="bg-white p-6 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4 flex-shrink-0">
            ...
            <div className="flex items-center gap-4 text-center sm:text-left">
              <div>
                ...
                <span className="font-bold text-gray-700">{totalQuantity} шт.</span>
              </div>
              <div className="w-px h-8 bg-gray-200 hidden sm:block"></div>
              <div>
                <span className="text-xs text-gray-400 block mb-0.5">До сплати</span>
                <span className="text-xl font-black text-[#4B32B1]">{totalPrice.toLocaleString('uk-UA')} ₴</span>
              </div>
            </div>
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <button 
                onClick={onClose}
                className="px-6 py-2.5 rounded-md font-semibold text-gray-500 bg-gray-100 hover:bg-gray-200 transition-colors flex-1 sm:flex-none text-sm"
              >
                Продовжити
              </button>
              <button 
                ...
                className="px-8 py-2.5 rounded-md font-semibold text-white bg-[#4B32B1] hover:bg-[#3b2888] transition-colors shadow-sm flex-1 sm:flex-none text-sm"
              >
                В кошик
              </button>
            </div>
