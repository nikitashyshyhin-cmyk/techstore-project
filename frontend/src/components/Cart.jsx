...
// ФУНКЦІЯ ВИДАЛЕННЯ ТОВАРУ
  ...
  // Динамічне обчислення загальної кількості та суми
      ...
      <div className="max-w-[1000px] mx-auto">
        ...
        {isLoading ? (
          ...
          <>
              ...
              {cartItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Кількість */}
                    <div className="flex flex-col items-center sm:items-start w-full sm:w-auto min-w-[80px] sm:border-l sm:border-gray-100 sm:pl-6">
            ...
            {/* ОНОВЛЕНИЙ БЛОК ІЗ ПІДСУМКОМ ТА КНОПКАМИ */}
            <div className="mt-8 bg-white p-6 rounded-md shadow-sm border border-gray-200 flex flex-col lg:flex-row justify-between items-center gap-6">
              ...
              <Link 
                to="/products"
                className="px-6 py-2.5 rounded-md font-semibold text-gray-500 hover:text-gray-800 transition-colors w-full lg:w-auto text-center order-3 lg:order-1"
              >
                ← До каталогу
              </Link>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 lg:gap-8 w-full lg:w-auto order-1 lg:order-2 text-center sm:text-left bg-gray-50 lg:bg-transparent p-4 lg:p-0 rounded-md">
                <div>
                  ...
                  <span className="font-bold text-gray-800">{totalQuantity} шт.</span>
                </div>
                <div className="hidden sm:block w-px h-8 bg-gray-200"></div>
                <div>
                  <span className="text-sm text-gray-500 mr-2">До сплати:</span>
                  <span className="text-2xl font-black text-[#4B32B1]">{totalPrice.toLocaleString('uk-UA')} ₴</span>
                </div>
              </div>

              <button 
                onClick={() => navigate('/checkout')}
                className="px-10 py-3.5 rounded-md font-semibold text-white bg-[#4B32B1] hover:bg-[#3b2888] transition-colors shadow-md w-full lg:w-auto order-2 lg:order-3"
              >
                Оформити замовлення
              </button>
            </div>
